...
import { ArrowLeft, GraduationCap, CheckCircle2, BookOpen, Clock } from "lucide-react"
...
export default function GraduationPage() {
  ...
  const totalCredits = histories.reduce((sum, h) => sum + (h.course?.credits ?? 3), 0)
        ...
        <div className="flex flex-col gap-8">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-lg border border-border bg-card p-5">
              ...
              <p className="text-3xl font-bold text-foreground">{totalCredits} <span className="text-sm font-normal text-muted-foreground">학점</span></p>
            </div>
            <div className="rounded-lg border border-border bg-card p-5">
              <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-3xl font-bold text-foreground">{histories.length} <span className="text-sm font-normal text-muted-foreground">과목</span></p>
            </div>
          </div>
                  ...
                  {histories.map((h) => (
                    <div key={h.id} className="p-4 hover:bg-muted/30 transition-colors">
                      <div className="flex justify-between items-start mb-1">
                        <h3 className="text-sm font-medium text-foreground">{h.course?.course_name || "알 수 없는 과목"}</h3>
