...
텍스트에 일본어·한자 등 한국어/영어가 아닌 문자가 포함된 경우, 이는 PDF 인코딩 오류입니다. 해당 문자는 무시하고 주변 문맥으로 의미를 파악하여 한국어로 작성하세요.
반환하는 JSON 값에는 반드시 한국어와 영어(전문용어)만 사용하세요. 일본어·한자는 절대 출력하지 마세요.
