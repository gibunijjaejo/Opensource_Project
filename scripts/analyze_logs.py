"""
Docker 컨테이너 에러 로그 수집 → Groq AI 분석 → Discord Embed 카드 전송
...
추후 팀서버에 Claude Code 설치·인증 완료 시, analyze_with_groq() 대신
analyze_with_claude_cli()를 호출하도록 한 줄만 바꾸면 됩니다.
"""
...
def analyze_with_claude_cli(filtered_logs: str) -> str:
    """
    팀서버에 Claude Code가 설치·인증된 경우 사용하는 대안 함수.
    analyze_with_groq() 대신 이 함수를 호출하면 됩니다.
    """
    prompt = ANALYSIS_USER_TEMPLATE.format(logs=filtered_logs[:6000])
    try:
        result = subprocess.run(
            ["claude", "-p", ANALYSIS_SYSTEM_PROMPT + "\n\n" + prompt],
            capture_output=True,
            text=True,
            timeout=60,
        )
        ...
        return f"(claude CLI 오류: {result.stderr.strip()})"
    except FileNotFoundError:
        return "(claude CLI를 찾을 수 없습니다. Groq 방식으로 전환하세요)"
    except subprocess.TimeoutExpired:
        return "(claude CLI 타임아웃)"
